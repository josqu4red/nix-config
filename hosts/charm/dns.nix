...
in
{
  ...
  systemd.tmpfiles.rules = [
    ''f /etc/named/amiez.xyz-extra - - - - ; papers IN CNAME  gluon.in.amiez.xyz.''
  ];
  ...
  services.bind =
    let
      ...
      record =
        type: name: data:
        "${padStr 25 name} IN ${padStr 6 type} ${data}";
      aRecord = record "A";
      ...
      aliases = listToAttrs (
        concatLists (mapAttrsToList (n: v: map (alias: (nameValuePair alias n)) v.aliases) hosts)
      );

...
let
  ...
  xdgFilesFromDir =
    target: path:
    let
      files = filterAttrs (n: v: v == "regular") (readDir path);
    in
    mapAttrs' (k: v: nameValuePair "${target}/${k}" { source = "${path}/${k}"; }) files;
...
in
{
  ...
  systemd.user.targets.hm-graphical-session.Unit.Requires = lib.mkForce [

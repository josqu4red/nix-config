...
let
  ...
  ifExists = path: optional (pathExists path) path;
  ...
  buildHomeConfigs = concatMapAttrs (
    hostname: v:
    let
      ...
      map (
        ...
        nameValuePair "${username}@${hostname}" (homeManagerConfiguration {
          ...
          extraSpecialArgs = {
            ...
            inherit (v) stateVersion;
          };
          modules = [ ../users/${username}/home.nix ] ++ ifExists ../users/${username}/${hostname}.nix;
        })
      ) v.users
    )
  );
